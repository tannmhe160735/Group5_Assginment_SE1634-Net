...
//        }
    ...
    public List<OrderDetail2> GetOrderByMonth1() {
        List<OrderDetail2> orderDetails = new ArrayList<OrderDetail2>();
        try {
            String sql = "select P.product_id,P.price, D.quantity, O.order_id \n"
            ...
            while (rs.next()) {
                Product pro = Product.builder()
                        .product_id(rs.getInt("P.product_id"))
                        .price(rs.getFloat("P.price")).build();
                OrderDetail2 order = OrderDetail2.builder()
                        .order_id(rs.getInt("P.product_id"))
                        .product_id(pro)
                        .quantity(rs.getInt("D.quantity")).build();
