...
public class Nhat_PostDBContext extends Nhat_DBContext<Post> {
    ...
    public boolean editThisPostExceptIMGpath(Post thisPost) {
        try {
            ...
            prstm.setString(3, thisPost.getContent1());
            prstm.setString(4, thisPost.getContent2());
            prstm.setDate(5, thisPost.getPost_date());
            prstm.setInt(6, thisPost.getId());
            prstm.executeUpdate();
        } catch (SQLException ex) {
            Logger.getLogger(Nhat_PostDBContext.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    public boolean hidePost(Post thisPost) {
        try {
            String sql = "UPDATE [dbo].[post]\n"
                    ...
                    + " WHERE [post].post_id=?";
            PreparedStatement prstm = connection.prepareStatement(sql);
            prstm.setString(1, "[HIDDEN]"+thisPost.getPost_title());
            prstm.setInt(2, thisPost.getId());
            prstm.executeUpdate();
        } catch (SQLException ex) {
            Logger.getLogger(Nhat_PostDBContext.class.getName()).log(Level.SEVERE, null, ex);
        }
